// ─── Pure project-file builders (no disk I/O) ────────────────────────────────
// This module intentionally avoids `fs`/`path` so it can be bundled for the
// browser (used by the in-app code preview) as well as required directly by
// the Electron main process when writing a generated project to disk.
...
// ─── Node.js / Express file templates ────────────────────────────────────────
// Plain string builders (not a templating engine) so this module has no
// dependency that assumes a filesystem — it needs to run in the browser
// (in-app code preview) as well as the Electron main process.

function buildAppJs({ projectName, routes, withDb }) {
  const dotenv = withDb ? `require('dotenv').config();\n` : '';
  return `${dotenv}const express = require('express');
const app = express();
...
function buildRouteFile({ controllerName, endpoints }) {
  const routeLines = endpoints.map(ep =>
...
function buildControllerFile({ endpoints }) {
...
function buildNodeFiles(apis, projectName, version = 'express4', dbConfig = null) {
  ...
  files['README.md'] = buildReadme({ projectName, apis, version });
  ...
  for (const [resource, endpoints] of Object.entries(groups)) {
    ...
    files[`routes/${resource}Route.js`] = buildRouteFile({
    ...
    if (dbConfig && hasTableEndpoint) {
      ...
      files[`controllers/${resource}Controller.js`] = buildControllerFile({ endpoints: enriched });
    }
